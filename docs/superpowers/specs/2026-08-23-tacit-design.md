...
- Ships with sensible defaults from the ergonomics report's example maps, all *disabled* except the workhorse core so first-run is calm (defaults **revision 8**, 2026-08-24): thumb–middle tap → next app, thumb–index tap → previous app (palm tilts do the same), thumbs-up → focus text input, thumb-swipe-on-index → undo/redo (⌘Z / ⇧⌘Z), index-point *hold* → Fn (push-to-talk dictation), victory → *toggle* Fn (hands-free dictation — moved off thumb–ring/pinky tap in revision 5, since that tap physically overlaps the index-point pose). An open-palm tilt flips apps: lean right → next, lean left → previous (`.switchApp`, direct `NSRunningApplication.activate`, never ⌘Tab) — revision 7 moved this off the four hand swipes, which weren't being reliably detected; the swipes stay recognized and bindable but ship *disabled*, keeping their old `.switchApp` suggestion. Revision 8 gave the thumb-index/thumb-middle taps the same `.switchApp` treatment as the palm tilts (⌘C/⌘V are still bindable manually via the Keystroke action kind, just no longer the factory default). Thumb–ring/pinky tap ships *disabled* with no suggested action. Every other dynamic gesture ships off. Default *values* are versioned separately from the wire format: a `DefaultsRevision` chain rewrites only bindings still exactly equal to the previous default. Loose fist (clutch) and open palm (disarm) are reserved system gestures and cannot be bound.
...
- Single window, three sections in fixed editorial order: **Workhorses** (report gestures 1–8: static, desk-resting), **Occasional** (9–17, plus the 2026-08-24 palm-tilt pair that replaced the hand swipes as the app-switch gestures: dynamic), **Deliberate** (18–20: free-air, rare). Section headers carry one-line editorial notes derived from the research ("Built for a resting arm. Use these for anything you do dozens of times an hour.").
...
- Repo: `~/Developer/tacit` (this repo; GitHub `breton-studio/tacit`, public). Xcode project generated in M1.
